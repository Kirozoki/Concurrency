...
class thread_pool_l_work_queue {
    private:
    ...
    thread_pool_l_work_queue() : done(false), joiner(threads) {
        const int thread_count = std::thread::hardware_concurrency();
        try {
            for (int i = 0; i < thread_count; ++i) {
                threads.push_back(
                    std::thread(&thread_pool_l_work_queue::worker_thread, this));
